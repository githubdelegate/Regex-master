...
public class Match : MatchProtocol {
    ...
    public func allGroupElement() -> [String] {
        var g: [String] = []
        var i = 0
        while let ele = group(at: i) {
            g.append(ele)
            i = i + 1
        }
        return g
    ...
    ///  直接查询match
    public func groupWith(name: String) -> String? {
        let res = self.group(named: name)
        if  res != nil {
            return res
        }
        
        if #available(iOS 11.0, *) {
            let range = try? self.match.range(withName: name).asRange(ofString: source)
            return range.map { range in
               String(source[range])
            }
        } else {
